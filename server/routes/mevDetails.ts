...
export function registerMevDetailsRoutes(app: Express) {
  app.get(
    '/api/mev/atomic',
    async (req: Request, res: Response<AtomicArbResponse | ErrorResponse>) => {
      try {
  ...
  app.get(
    '/api/mev/cexdex',
    async (req: Request, res: Response<CexDexQuoteResponse | ErrorResponse>) => {
      try {
        const txHash = req.query.tx_hash as string;
  ...
  app.get(
    '/api/mev/liquidations',
    async (req: Request, res: Response<LiquidationResponse | ErrorResponse>) => {
      try {
        const txHash = req.query.tx_hash as string;
