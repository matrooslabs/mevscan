...
import { Request, Response, BlockListItem, ErrorResponse, Transaction } from './types';
...
export function registerLatestRoutes(app: Express) {
  app.get(
    ...
    async (req: Request, res: Response<Transaction[] | ErrorResponse>) => {
      try {
  ...
  app.get(
    '/api/latest-blocks',
    async (req: Request, res: Response<BlockListItem[] | ErrorResponse>) => {
      try {
        const limit = parseInt(req.query.limit as string) || 20;
