...
import { formatRelativeTime, formatEthValue } from './types';
...
export function registerEntitiesRoutes(app: Express) {
  app.get(
    ...
    async (req: Request<{ blockNumber: string }>, res: Response<Block | ErrorResponse>) => {
      try {
  ...
  app.get(
    ...
    async (req: Request<{ transactionId: string }>, res: Response<Transaction | ErrorResponse>) => {
      try {
      ...
      `;
              ...
              if (Array.isArray(tokenDeltasArray)) {
                for (const tokenDelta of tokenDeltasArray) {
                  ...
                  if (Array.isArray(tokenDelta) && tokenDelta.length >= 3) {
                    ...
                    const delta = Array.isArray(tokenInfo)
                      ? Number(tokenDelta[1] || 0)
                      : Number(tokenDelta[3] || 0);
                    const deltaUsd = Array.isArray(tokenInfo)
                      ? Number(tokenDelta[2] || 0)
                      : Number(tokenDelta[4] || 0);
        ...
        const transaction: Transaction = {
          ...
          expressLaneRound: row.express_lane_round,
          from:
            row.from.length > 14 ? `${row.from.slice(0, 10)}...${row.from.slice(-6)}` : row.from,
          to: row.to
            ? row.to.length > 14
  ...
  app.get(
    ...
    async (req: Request<{ address: string }>, res: Response<Address | ErrorResponse>) => {
      try {
      ...
      `;
        ...
        const contractCheckData = await contractCheckResult.json<
          Array<{
            mev_contract: string | null;
            eoa: string;
          }>
        ...
        `;
          ...
          const eoaData = await eoaResult.json<
            Array<{
              tx_hash: string;
              block_number: number;
              tx_index: number;
        ...
        const overallStatsData = await overallStatsResult.json<
          Array<{
            total_profit: number;
            total_bribe: number;
        ...
        const mevTypeData = await mevTypeResult.json<
          Array<{
            mev_type: string;
        ...
        const statistics: AddressStatistics = {
          ...
          timeboostedCount:
            overallStatsData.length > 0 ? overallStatsData[0].timeboosted_count || 0 : 0,
          mevTypeBreakdown: mevTypeData.reduce(
            (acc, row) => {
        ...
        if (totalCount > 0) {
          ...
          try {
            ...
            const blockRangeData = await blockRangeResult.json<
              Array<{
                min_block: number;
                max_block: number;
              }>
            ...
            if (
              blockRangeData.length > 0 &&
              blockRangeData[0].min_block &&
              blockRangeData[0].max_block
            ) {
