...
export async function publishExpressLaneTransactions(
  ...
  if (lastStoredBlockNumber === null || lastStoredTxIndex === null) {
    ...
    if (!message || message.length === 0) {
      lastStoredBlockNumber = await getLastStoredBlockNumberTxIndex(
        clickhouseClient
      );
      lastStoredTxIndex = 0;
    } else {
      const expressLaneTransactionData =
        message[0] as unknown as ExpressLaneTransaction[];
      lastStoredBlockNumber =
        expressLaneTransactionData[expressLaneTransactionData.length - 1]!
          .blockNumber;
      lastStoredTxIndex =
        expressLaneTransactionData[expressLaneTransactionData.length - 1]!
          .txIndex;
