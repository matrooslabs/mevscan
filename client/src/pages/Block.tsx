...
function Block() {
  ...
  const { data: blockData, isLoading, error } = useBlock(block_number || '');
  ...
  if (error) {
    return (
      <div className="block-page-container">
        <div className="error-state">Error loading block: {error.message}</div>
      </div>
    );
  }
          ...
          <div className="section-content">
            <div className="overview-grid">
              ...
              <div className="overview-item">
                ...
                <span className="overview-value">
                  ...
                  {block.expressLaneTxns !== undefined && block.expressLaneTxns > 0 && (
                    <span className="express-lane-badge">
                      {' '}
                      ({block.expressLaneTxns} Express Lane)
                    </span>
                  )}
                </span>
              </div>
              <div className="overview-item">
                ...
                <span className="overview-value">
                  {typeof block.gasUsed === 'string'
                    ? block.gasUsed
                    : block.gasUsed?.toLocaleString() || '0'}
                </span>
              </div>
              <div className="overview-item">
          ...
          <div className="block-section">
            <div className="section-header">
              ...
              <span className="transaction-count">
                {block.bundles.length} {block.bundles.length === 1 ? 'transaction' : 'transactions'}
              </span>
            </div>
            <div className="section-content">
                  ...
                  {block.bundles.map((bundle) => (
                    <div key={bundle.txHash} className="transaction-row">
                      ...
                      <div className="transaction-eoa monospace">
                        <Link to={`/address/${bundle.eoa}`}>{formatAddress(bundle.eoa)}</Link>
                      </div>
                      <div className="transaction-mev-contract monospace">
                        {bundle.mevContract ? formatAddress(bundle.mevContract) : '—'}
                      </div>
                      <div className="transaction-mev-type">{bundle.mevType || '—'}</div>
                      <div className="transaction-profit">
                        $
                        {bundle.profitUsd.toLocaleString(undefined, {
