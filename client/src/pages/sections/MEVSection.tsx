...
): { transformedData: Record<string, string | number>[]; lineConfigs: LineConfig[] } => {
  if (!data || data.length === 0) {
    return { transformedData: [], lineConfigs: [] }
  }
  
  const protocols = Array.from(new Set(data.map((item) => String(item.proto)))).sort()
  const times = Array.from(new Set(data.map((item) => String(item.time)))).sort()
