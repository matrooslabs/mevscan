...
function generateMockData(): LaneTimeSeriesDataPoint[] {
  ...
  for (let i = 14; i >= 0; i--) {
    ...
    data.push({
      ...
      expressLane: Math.floor(Math.random() * 800000) + 200000,
    });
  }
...
function TradeVolumeChart({ data }: TradeVolumeChartProps) {
      ...
      tooltip: {
        ...
        formatter: (params: unknown) => {
          ...
          const lines = items.map((item) => {
            ...
            return `${colorDot}${item.seriesName}: $${item.value.toLocaleString()}`;
          });
